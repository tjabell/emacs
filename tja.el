...
(defun m/gsi:get-ticket-dependencies (ticket-ids)
  ...
  (let* ((formatted-ids (mapconcat 'number-to-string ticket-ids ","))
         (wql (format *WQL-FOR-TICKET-DEPENDENCIES* formatted-ids))
...
(defun m/gsi/emacs:get-and-print-dependencies (ticket-ids)
  "Retrieve and print dependencies for the given ticket IDs."
  (let ((dependencies (m/gsi:get-ticket-dependencies ticket-ids)))
    (if dependencies
...
;;; TODO: Create workitem/story?
(defun azure--add-user-story (title description)
  ;; Not sure if there is an API for this, maybe only for updating work items
  ;; https://learn.microsoft.com/en-us/rest/api/azure/devops/wit/work-items/create?view=azure-devops-rest-7.1&tabs=HTTP
  ;;   POST https://dev.azure.com/fabrikam/GoddardSystemsIT/_apis/wit/workitems/story?api-version=7.1
...
;;; WIP, getting some errors.  I think I want to use this from a sly-scratch buffer but thinking about it still
(defun m/sly:eval-and-print-to-temp-buffer (string)
...
;; [[file:tja.org::*Changelog Generation][Changelog Generation:1]]
...
(defun m/gsi/emacs:changelog:insert-release ()
  ...
  (let* ((date (format-time-string "%Y-%m-%d" (org-read-date nil t nil "Select a date")))
    ...
    (if (fboundp 'markdown-mode)
        (markdown-mode))))
;; Changelog Generation:1 ends here
