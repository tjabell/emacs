...
;;;###autoload
(defun m/esa:copy-file-to-qa ()
  "Copy the current buffer's file to the same relative location on /mnt/dev-dav."
  (interactive)
  (when (not (file-directory-p "/mnt/qa-dav/www.extendedstayamerica.com"))
      (shell-command (concat "echo " (shell-quote-argument (read-passwd "Password? ")) " | sudo -S mount -t davfs -o uid=trevor,gid=trevor https://cms.qa.bws.esa.com/webdav/live/1 /mnt/qa-dav")))
  (let* ((current-file (buffer-file-name))
         (project-root (project-root (project-current)))
         (relative-path (replace-regexp-in-string "src" "application" (file-relative-name current-file project-root)))
         (target-path (expand-file-name relative-path "/mnt/qa-dav/www.extendedstayamerica.com/")))
    (make-directory (file-name-directory target-path) t)
    (copy-file current-file target-path t)
    (message "File copied to %s" target-path)))
