...
#+begin_src emacs-lisp :results raw
  ...
  (defun m/gsi/emacs:changelog:insert-release ()
    "Insert a new release entry into the changelog file."
    (interactive)
    (let* ((date (format-time-string "%Y-%m-%d" (org-read-date nil t nil "Select a date")))
           (version (read-string "Enter version string: "))
           (tickets (let ((input (read-string "Enter ticket numbers (space-separated): ")))
                      (split-string input " ")))
           ...
           (release-markdown (concat
                              ...
                              (format "### Tickets\n")
                              (mapconcat (lambda (ticket) (format "#%s" ticket)) tickets "\n")
      ...
      (search-forward "Canonical URL:") ; Find the header
      (forward-line 3)            ; Skip eight lines after header
