...
(defun my-esa:replace-url-with-dev ()
  (interactive)
  (let ((regex "http\[s\]*://.*?/")
        (replacement "http://cms.dev.bws.esa.com/"))
    (while (re-search-forward regex nil t)
      (replace-match replacement))))
...
(defun my:esa-vtl-copy-to-local ()
  (let* ((dest-suffix (substring
    ...
    (message "copying %s %s"  buffer-file-name (concat "/mnt/local-dav/www.extendedstayamerica.com/application/" dest-suffix))))

(defun my:esa-vtl-copy-to-dev ()
  (let* ((dest-suffix (substring
                       (buffer-file-name)
                       (length "/home/trevor/projects/extended_stay/src/frontend/src/"))))
    (message "copying: %s %s" buffer-file-name (concat "/mnt/dev-dav/www.extendedstayamerica.com/application/" dest-suffix))
    (copy-file buffer-file-name (concat "/mnt/dev-dav/www.extendedstayamerica.com/application/" dest-suffix) t)))
